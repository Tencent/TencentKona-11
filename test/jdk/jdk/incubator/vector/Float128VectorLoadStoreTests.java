...
public class Float128VectorLoadStoreTests extends AbstractVectorTest {
    ...
    static void loadStoreMaskArray(IntFunction<float[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < a.length; i += SPECIES.length()) {
                FloatVector<Shapes.S128Bit> av = SPECIES.fromArray(a, i, vmask);
                av.intoArray(r, i);
            }
        }
    ...
    static void loadStoreIndexMapArray(IntFunction<float[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < a.length; i += SPECIES.length()) {
                FloatVector<Shapes.S128Bit> av = SPECIES.fromArray(a, 0, im, i);
                av.intoArray(r, i);
            }
        }
...
*/
    ...
    static void loadStoreIndexMapMaskArray(IntFunction<float[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < a.length; i += SPECIES.length()) {
                FloatVector<Shapes.S128Bit> av = SPECIES.fromArray(a, 0, vmask, im, i);
                av.intoArray(r, i);
            }
        }
...
*/
    ...
    static void loadStoreByteBuffer(IntFunction<float[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < l; i += s) {
                FloatVector<Shapes.S128Bit> av = SPECIES.fromByteBuffer(a, i);
    ...
    static void loadStoreByteBufferMask(IntFunction<float[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < l; i += s) {
                FloatVector<Shapes.S128Bit> av = SPECIES.fromByteBuffer(a, i, vmask);
                av.intoByteBuffer(r, i);
            }
        }
        Assert.assertEquals(a.position(), 0, "Input buffer position changed");
