...
public class Long64VectorTests extends AbstractVectorTest {
   ...
   static final IntFunction<long[]> fr = (vl) -> {
    ...
    @Test(dataProvider = "longBinaryOpProvider")
    static void gatherLong64VectorTests(IntFunction<long[]> fa, IntFunction<long[]> fb) {
        long[] a = fa.apply(SPECIES.length()); 
    ...
    @Test(dataProvider = "longBinaryOpProvider")
    static void scatterLong64VectorTests(IntFunction<long[]> fa, IntFunction<long[]> fb) {
        long[] a = fa.apply(SPECIES.length()); 
        long[] bb = fb.apply(SPECIES.length());
        int[] b = new int[bb.length];
        for (int i = 0; i < bb.length; i++) {
