...
public class Short256VectorLoadStoreTests extends AbstractVectorTest {
    static final ShortVector.ShortSpecies<Shapes.S256Bit> SPECIES =
                ShortVector.species(Shapes.S_256_BIT);
    ...
    static void loadStoreMaskArray(IntFunction<short[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < a.length; i += SPECIES.length()) {
                ShortVector<Shapes.S256Bit> av = SPECIES.fromArray(a, i, vmask);
                av.intoArray(r, i);
            }
        }
...
*/
    ...
    static void loadStoreByteBuffer(IntFunction<short[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < l; i += s) {
                ShortVector<Shapes.S256Bit> av = SPECIES.fromByteBuffer(a, i);
    ...
    static void loadStoreByteBufferMask(IntFunction<short[]> fa,
        ...
        for (int ic = 0; ic < INVOC_COUNT; ic++) {
            for (int i = 0; i < l; i += s) {
                ShortVector<Shapes.S256Bit> av = SPECIES.fromByteBuffer(a, i, vmask);
                av.intoByteBuffer(r, i);
            }
        }
        Assert.assertEquals(a.position(), 0, "Input buffer position changed");
