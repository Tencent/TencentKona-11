...
public class Float128VectorTests extends AbstractVectorTest {
   ...
   static final IntFunction<float[]> fr = (vl) -> {
    ...
    @Test(dataProvider = "floatBinaryOpProvider")
    static void gatherFloat128VectorTests(IntFunction<float[]> fa, IntFunction<float[]> fb) {
        float[] a = fa.apply(SPECIES.length()); 
    ...
    @Test(dataProvider = "floatBinaryOpProvider")
    static void scatterFloat128VectorTests(IntFunction<float[]> fa, IntFunction<float[]> fb) {
        float[] a = fa.apply(SPECIES.length()); 
        float[] bb = fb.apply(SPECIES.length());
        int[] b = new int[bb.length];
        for (int i = 0; i < bb.length; i++) {
