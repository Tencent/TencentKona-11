...
public class SchedulerCocktail {
    ...
    public void testStartVirtualThread3() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        Thread.builder().virtual(SCHEDULER_1).name("testStartVirtualThread3").task(() -> {
            try {
                Thread.startVirtualThread(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                }).join();
    ...
    public void testBuilderNoScheduler3() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        Thread.builder().virtual(SCHEDULER_1).name("testBuilderNoScheduler3").task(() -> {
            try {
                Thread.builder().virtual().task(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                }).start().join();
    ...
    public void testBuilderCustomScheduler1() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        Thread.builder().virtual(SCHEDULER_1).name("testBuilderCustomScheduler1").task(() -> {
            ref.set(scheduler(Thread.currentThread()));
        }).start().join();
        assertTrue(ref.get() == SCHEDULER_1);
    }
    ...
    public void testBuilderCustomScheduler2() throws Exception {
        ...
        Thread.startVirtualThread(() -> {
            try {
                Thread.builder().virtual(SCHEDULER_1).name("testBuilderCustomScheduler2").task(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                }).start().join();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void testBuilderCustomScheduler3() throws Exception {
        ...
        Thread.builder().virtual(SCHEDULER_1).task(() -> {
            try {
                Thread.builder().virtual(SCHEDULER_2).name("testBuilderCustomScheduler3").task(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                }).start().join();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void testThreadFactoryNoScheduler3() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        Thread.builder().virtual(SCHEDULER_1).name("testThreadFactoryNoScheduler3").task(() -> {
            try {
                ThreadFactory factory = Thread.builder().virtual().factory();
                Thread thread = factory.newThread(() -> {
                    ref.set(scheduler(Thread.currentThread()));
    ...
    public void testThreadFactoryCustomScheduler1() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        ThreadFactory factory = Thread.builder().virtual(SCHEDULER_1).name("testThreadFactoryCustomScheduler1").factory();
        Thread thread = factory.newThread(() -> {
            ref.set(scheduler(Thread.currentThread()));
        });
        thread.start();
    ...
    public void testThreadFactoryCustomScheduler2() throws Exception {
        ...
        Thread.startVirtualThread(() -> {
            try {
                ThreadFactory factory = Thread.builder().virtual(SCHEDULER_1).name("testThreadFactoryCustomScheduler2").factory();
                Thread thread = factory.newThread(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                });
                thread.start();
    ...
    public void testThreadFactoryCustomScheduler3() throws Exception {
        AtomicReference<Executor> ref = new AtomicReference<>();
        Thread.builder().virtual(SCHEDULER_1).name("testThreadFactoryCustomScheduler2_1").task(() -> {
            try {
                ThreadFactory factory = Thread.builder().virtual(SCHEDULER_2).name("testThreadFactoryCustomScheduler2").factory();
                Thread thread = factory.newThread(() -> {
                    ref.set(scheduler(Thread.currentThread()));
                });
                thread.start();
